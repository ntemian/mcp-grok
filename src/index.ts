...
server.tool(
  'grok_chat',
  'Send a chat completion to xAI Grok. Has real-time information from X/Twitter.',
  {
  ...
  async (params: { messages: ChatMessage[]; model?: string; temperature?: number; max_tokens?: number }) => {
    try {
...
server.tool(
  'grok_complete',
  'Simple one-shot completion with Grok.',
  {
...
server.tool(
  'grok_fast',
  'Fast completion using Grok 3 Fast. Optimized for speed.',
  {
...
server.tool(
  'grok_fun',
  'Grok with its signature witty, irreverent personality enabled. Good for humor and creative tasks.',
  {
    prompt: z.string().describe('The prompt'),
    spicy: z.boolean().optional().describe('Extra spicy mode (more irreverent). Default: false'),
  },
  async (params: { prompt: string; spicy?: boolean }) => {
    try {
...
server.tool(
  'grok_code',
  'Generate code using Grok. Strong at reasoning through complex problems.',
  {
...
server.tool(
  'grok_reason',
  'Use Grok for complex reasoning and problem-solving. Shows step-by-step thinking.',
  {
    problem: z.string().describe('The problem or question requiring deep reasoning'),
    context: z.string().optional().describe('Additional context'),
  },
  async (params: { problem: string; context?: string }) => {
    try {
...
server.tool(
  'grok_realtime',
  'Ask Grok about current events. Grok has access to real-time information from X/Twitter.',
  {
...
server.tool(
  'grok_vision',
  'Analyze images using Grok vision. Pass an image URL (web URL or tweet media) and a question about it.',
  {
    image_url: z.string().describe('URL of the image to analyze'),
    prompt: z.string().describe('What to analyze or ask about the image'),
    model: z.string().optional().describe('Model to use. Default: grok-4-1-fast-reasoning'),
  },
  async (params: { image_url: string; prompt: string; model?: string }) => {
    try {
...
server.tool(
  'grok_models',
  'List available Grok models and their capabilities.',
  {},
  async () => {
    const models = `
xAI Grok Models:
...
server.tool(
  'grok_json',
  'Get structured JSON output from Grok.',
  {
